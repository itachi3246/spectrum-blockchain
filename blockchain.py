...
class Blockchain:
            ...
            with open('blockchain-{}.txt'.format(self.node_id), mode='r') as f:
                ...
                for block in blockchain:
                    converted_tx = [Transaction(
                        tx['node_id'], tx['snr'], tx['sender'], tx['recipient'], tx['signature'], tx['amount'])
    ...
    def get_balance(self, sender=None):
        ...
        if sender == None:
            ...
            participant = sender
        # Fetch a list of all sent coin amounts for the given person (empty lists are returned if the person was
        # NOT the sender)
    ...
    def add_transaction(self, node_id, snr, recipient, sender, signature, amount=1.0, is_receiving=False):
        ...
        # transaction = {
        #     'node_id': port number,
        #     'snr' : SNR of received signals in dB,
        #     'sender': sender,
        #     'recipient': recipient,
        #     'amount': amount
        # }
        ...
        transaction = Transaction(node_id, snr, sender, recipient, signature, amount)
                ...
                for node in self.__peer_nodes:
                    ...
                    try:
                        response = requests.post(url, json={
                                                 'node_id': node_id, 'snr': snr, 'sender': sender,
                                                 'recipient': recipient, 'amount': amount, 'signature': signature})
    ...
    def mine_block(self):
        ...
        reward_transaction = Transaction(
            self.node_id, None, 'MINING', self.public_key, '', MINING_REWARD)
        # Copy transaction instead of manipulating the original open_transactions list
        # This ensures that if for some reason the mining should fail, we don't have the reward transaction stored in
    ...
    def add_block(self, block):
        ...
        transactions = [Transaction(
            tx['node_id'], tx['snr'], tx['sender'], tx['recipient'], tx['signature'], tx['amount']) for tx in block['transactions']]
